...
public class CalcularTests {
	...
	public void calculos() {
		...
		assertEquals("@delucas 3", jenkins.escuchar("@jenkins cuánto es 1 + 2",USUARIO));

		assertEquals("@delucas 3.5", jenkins.escuchar("@jenkins cuánto es 1.5 + 2",USUARIO));

		assertEquals("@delucas 1", jenkins.escuchar("@jenkins cuánto es 5 - 2 * 2",USUARIO));

		assertEquals("@delucas 10", jenkins.escuchar("@jenkins cuánto es el 10% de 100",USUARIO));

		assertEquals("@delucas 42", jenkins.escuchar("@jenkins cuánto es el 17 + 5 ^ 2",USUARIO));
	...
	public void calculosCompuestos() {
		...
		assertEquals("@delucas -6", jenkins.escuchar("@jenkins cuánto es (4-8)*2 + 4 / ( 1 + 1)",USUARIO));
