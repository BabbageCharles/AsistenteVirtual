...
public class Fecha implements AsistenteEscucha {
	...
	public static String USUARIO = "delucas"; 
	...
	public static Date FECHA_HORA;// = new GregorianCalendar(2018, 3, 1, 15, 15,
									// 0).getTime();
	...
	public static String darFecha(String s) {
		...
		if (s.contains("pasaron")) {
			...
			long result=Fecha.diasDesde(s);
			if(result !=0) {
				String f = "entre el " + Fecha.convertCalendarToString(d1) + " y el " + Fecha.convertCalendarToString(d2)
				+ " pasaron " + result + " días";
				return "@" + USUARIO + " " + f;	
			}else
				return "Esa fecha no paso todavia";
		}

		if (s.contains("faltan")) {
			long result= Fecha.tiempoHasta(s);
			if(result !=0) {
				String f = "faltan " + Fecha.tiempoHasta(s) + " días";
				return "@" + USUARIO + " " + f;
			}
			else
